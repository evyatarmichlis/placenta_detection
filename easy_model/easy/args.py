...
python data_collection.py --epochs 30 --milestones 10 --dataset MNIST --feature-maps 8
To train MNIST with 10% database and 99.31% accuracy (10 minutes):
python data_collection.py --epochs 300 --dataset MNIST --dataset-size 6000 --model wideresnet --feature-maps 4 --skip-epochs 300
To train Fashion-MNIST with 96% accuracy (2 hours):
python data_collection.py --dataset fashion --mixup
To train CIFAR10 with 95.90% accuracy (1 hour):
python data_collection.py --mixup
To train CIFAR100 with 78.55% accuracy (93.54% top-5) (1hour):
python data_collection.py --mixup --dataset cifar100
To train CIFAR100 with 80.12% accuracy (94.70% top-5) (4h):
python data_collection.py --mixup --model wideresnet --feature-maps 16 --dataset CIFAR100
To train Omniglot (few-shot) with 99.85% accuracy (99.39% in 1-shot) (10minutes):
python data_collection.py --dataset omniglotfs --dataset-device cpu --feature-maps 16 --milestones 10 --epochs 30 --preprocessing "PEME"
To train CUBFS (few-shot) with 85.24% accuracy (68.14% in 1-shot) (2h):
python data_collection.py --dataset cubfs --mixup --rotations --preprocessing "PEME"
To train CIFARFS (few-shot) with 84.87% accuracy (70.43% in 1-shot) (1h):
python data_collection.py --dataset cifarfs --mixup --rotations --skip-epochs 300 --preprocessing "PEME"
To train CIFARFS (few-shot) with 86.83% accuracy (70.27% in 1-shot) (3h):
python data_collection.py --dataset cifarfs --mixup --model wideresnet --feature-maps 16 --skip-epochs 300 --rotations --preprocessing "PEME"
To train MiniImageNet (few-shot) with 80.43% accuracy (64.11% in 1-shot) (2h):
python data_collection.py --dataset miniimagenet --model resnet12 --gamma 0.2 --milestones 30 --epochs 120 --batch-size 128 --preprocessing 'EME'
To train MiniImageNet (few-shot) with rotations and 81.63% accuracy (65.64% in 1-shot) (2h):
python data_collection.py --dataset miniimagenet --model resnet12 --milestones 60 --epochs 240 --cosine --gamma 1 --rotations --skip-epochs 200
To train MiniImageNet (few-shot) with 83.18% accuracy (66.78% in 1-shot) (40h):
python data_collection.py --device cuda:012 --dataset miniimagenet --model S2M2R --lr -0.001 --milestones 0 --epochs 600 --feature-maps 16 --rotations --manifold-mixup 400 --skip-epochs 600 --preprocessing "PEME"
""", formatter_class=argparse.RawTextHelpFormatter)

### hyperparameters
parser.add_argument("--batch-size", type=int, default=32, help="batch size")
parser.add_argument("--batch-fs", type=int, default=10, help="batch size for few shot runs")
parser.add_argument("--feature-maps", type=int, default=64, help="number of feature maps")
parser.add_argument("--lr", type=float, default="0.01",
                    help="initial learning rate (negative is for Adam, e.g. -0.001)")
...
parser.add_argument("--milestones", type=str, default="100",
                    help="milestones for lr scheduler, can be int (then milestones every X epochs) or list. 0 means no milestones")
parser.add_argument("--gamma", type=float, default=-1., help="multiplier for lr at milestones")
parser.add_argument("--cosine", action="store_true",
                    help="use cosine annealing scheduler with args.milestones as T_max")
parser.add_argument("--mixup", action="store_true", help="use of mixup since beginning")
parser.add_argument("--mm", action="store_true",
                    help="to be used in combination with mixup only: use manifold_mixup instead of classical mixup")
...
parser.add_argument("--manifold-mixup", type=int, default="0",
                    help="deploy manifold mixup as fine-tuning as in S2M2R for the given number of epochs")
parser.add_argument("--temperature", type=float, default=1.,
                    help="multiplication factor before softmax when using episodic")
parser.add_argument("--ema", type=float, default=0,
                    help="use exponential moving average with specified decay (default, 0 which means do not use)")
...
parser.add_argument("--dataset-path", type=str,
                    default="/cs/usr/evyatar613/Desktop/josko_lab/downloads/depthData/class_dataset",
                    help="dataset path")
...
parser.add_argument("--skip-epochs", type=int, default="0",
                    help="number of epochs to skip before evaluating few-shot performance")
...
parser.add_argument("--dataset", type=str, default="depth", help="dataset to use")

parser.add_argument("--dataset-size", type=int, default=-1,
                    help="number of training samples (using a subset for classical classification, and reducing size of epochs for few-shot)")
...
parser.add_argument("--save-model", type=str, default="/cs/usr/evyatar613/josko_lab/easymodel/evyatar.pth",
...
parser.add_argument("--fine_tuning_strategy", type=str, default='', help="fine tune existing model")
...
parser.add_argument("--sample-aug", type=int, default=1,
                    help="number of versions of support/query samples (using random crop) 1 means no augmentation")
...
parser.add_argument("--transductive-n-iter", type=int, default=50,
                    help="number of iterations for few-shot transductive")
parser.add_argument("--transductive-n-iter-sinkhorn", type=int, default=200,
                    help="number of iterations of sinkhorn for few-shot transductive")
